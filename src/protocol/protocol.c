...
void pull_command(lfs_t *lfs, char *filename){
...
void push_command(lfs_t *lfs, const char *filename, uint32_t *size, uint32_t *expected_checksum) {
    lfs_file_t file;
...
void rm_command(lfs_t *lfs, char *filename) {
    struct lfs_info info;
...
void mv_command(lfs_t *lfs, char *old_filename, char *new_filename) {
    struct lfs_info info;
...
void handle_command(lfs_t *lfs, char *cmd) {
    ...
    else if (strcmp(args[0], "push") == 0)
    {
        ...
        push_command(lfs, filename, &size, &crc);
    }
        ...
        if (tud_cdc_available()) {
            ...
            for (size_t i = cmd_len-n; i < cmd_len; ++i) {
                if (cmd_buf[i] == EOT_CHAR) { // 4 is EOT
                    ...
                    handle_command(lfs, cmd_buf);  // Process command
